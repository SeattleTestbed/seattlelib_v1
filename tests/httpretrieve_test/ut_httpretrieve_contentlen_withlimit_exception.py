...
# match the acutal content length, when using file like obj read with limit (this only works
# when the content length is given by server's http header)
...
if callfunc == 'initialize':
  ...
  try:
    ...
    http_obj = httpretrieve_open('http://127.0.0.1:12345/')
    # use http read with limits to see if the httpretrieve keeps track of the total conent recieved
    # and checks it with the content length
    http_obj.read(10)
  ...
  #catch the right Exception if there is a different exception print failed_error_msg
    ...
    if 'Total length read with limit did not match the content length' not in str(e): 
